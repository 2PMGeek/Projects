function h = alluvialflow(data, left_labels, right_labels, chart_title, patch_colors, fsize, hkey, rh)
...
elseif nargin < 6
    ...
    rh = 0;
elseif nargin < 7
    alpha = repmat(alpha_s, 1, size(data, 1) );
    rh = 0;
elseif nargin == 7
    alpha = repmat(alpha_nh, 1, size(data, 1) );
    alpha(hkey) = alpha_h;
    rh = 0;
else
    alphaRH = repmat(alpha_h, 1, size(data, 1) );
    alphaR = repmat(alpha_nh, 1, size(data, 1) );
end
...
for k_left = 1:size(data, 1) % for each row
    ...
    if rh
        patch('XData', X, 'YData', Y, 'FaceColor', color, 'FaceAlpha', alphaR(k_left), 'EdgeColor', 'none');
        patch('XData', X(:,hkey), 'YData', Y(:,hkey), 'FaceColor', color, 'FaceAlpha', alphaRH(k_left), 'EdgeColor', 'none');
    else
        patch('XData', X, 'YData', Y, 'FaceColor', color, 'FaceAlpha', alpha(k_left), 'EdgeColor', 'none');
    end
end % for each row
...
text(zeros(1, size(data, 1)) - 0.01, ...
    ...
    left_labels, 'FontSize', fsize, 'HorizontalAlignment', 'center', 'VerticalAlignment', 'bottom', 'Rotation', 90);

% Place right labels
text(ones(1, size(data, 2)) + 0.01, ...
    right_category_points + right_category_sizes./2, ...
    right_labels, 'FontSize', fsize, 'HorizontalAlignment', 'center', 'VerticalAlignment', 'top', 'Rotation', 90);

title(chart_title, 'FontSize',fsize);
end % alluvialflow
